...
flags.DEFINE_string('rbm_layers', '300,200,100', 'Comma-separated values for the layers in the sdae.')
...
flags.DEFINE_string('rbm_learning_rate', '0.1,', 'Initial learning rate.')
flags.DEFINE_string('rbm_num_epochs', '10,', 'Number of epochs.')
flags.DEFINE_string('rbm_batch_size', '20,', 'Size of each mini-batch.')
...
flags.DEFINE_integer('finetune_batch_size', 20,  'Size of each mini-batch.')
...
if __name__ == '__main__':
    ...
    for i in range(300000):
        ...
        train_labels.append([1,1,1])

    for i in range(300000):
        train_data.append([0, 0, 0])
        train_labels.append([0,0,0])


    trX = np.array(train_data)
    trY = np.array(train_labels)
